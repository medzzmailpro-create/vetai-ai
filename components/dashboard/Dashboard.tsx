...
export default function Dashboard({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      try {
        ...
        // Load profile — check onboarding_completed + is_demo + clinic_id
        ...
        // Load real notifications (unread only) if clinic_id exists
        if (resolvedClinicId) {
          try {
          ...
          } catch { /* notifications table absent → keep empty */ }
        }
  ...
  // Update last_seen on mount and every 5 minutes
  useEffect(() => {
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#F5F5F3' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        <main style={{ padding: '20px 24px', flex: 1, overflowY: 'auto' }}>
          ...
          {page === 'facturation' && userRole === 'owner' && (
            <BillingPage clinicId={clinicId} />
          )}
          {page === 'facturation' && userRole === 'staff' && (
            <div style={{ background: 'white', border: '1px solid #EBEBEA', borderRadius: 12, padding: 40, textAlign: 'center' }}>
              ...
              <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 16, fontWeight: 700, color: '#2A2A28', marginBottom: 8 }}>Accès restreint</div>
              <div style={{ fontSize: 14, color: '#9E9E9B' }}>La facturation est réservée aux propriétaires de la clinique.</div>
            </div>
          )}
